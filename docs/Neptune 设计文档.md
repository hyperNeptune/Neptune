...
日志管理器负责生成日志，并将日志持久化存储，使用了 DiskManager 作为文件读写的接口。使用了先写日志（WAL）的方案。
...
这是一个简单的模块，只实现了两个关键的函数：检查点开始和检查点完成

**检查点开始**：事务管理器已经实现了 `BlockAll` 功能，检查点开始时，暂停所有事务并确保所有 WAL 日志和脏缓存池都已经持久化地写入磁盘

**检查点完成**：在确保检查点条件满足后，写入检查点日志，并调用 `ResumeAll` 以恢复所有事务

### 日志恢复器 LogRecovery

日志恢复器可以将数据库日志反序列化，除此之外，日志恢复器还提供了两个重要的接口：重做（Redo）和撤销（Undo）。重做操作将重新完成事务的行为，撤销操作将回滚事务的行为。

在有检查点的情况下，我们的恢复过程并不需要完整查阅整个日志。我们的恢复过程将分为两个阶段执行：重做阶段和撤销阶段

+ **重做阶段**：从检查点开始正向扫描，重演事务的更新
  + 从检查点日志中获取撤销列表
  + 对正常日志执行重做
  + 将事务开始日志加入撤销列表
