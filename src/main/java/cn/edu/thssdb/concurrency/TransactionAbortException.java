    ...
    public String getInfo() {
        switch (abortReason) {
            case UPGRADE_CONFLICT:
                return "Transaction " + txn_id + " aborted because another transaction is already waiting to upgrade its lock\n";
            case LOCK_ON_SHRINKING:
                return "Transaction " + txn_id + " aborted because it can not take locks in the shrinking state\n";
            case INCOMPATIBLE_UPGRADE:
                return "Transaction " + txn_id + " aborted because attempted lock upgrade is incompatible\n";
            ...
            case TABLE_UNLOCKED_BEFORE_UNLOCKING_ROWS:
                return "Transaction " + txn_id + " aborted because table locks dropped before dropping row locks\n";
        }
        return "";
    }
}
